...
function initMap() {
    map = new google.maps.Map(document.getElementById('map'), {
        zoom: 15,
        center: {lat: 40.735916, lng: -73.990574}
    });
...
let Marker = function(location) {
    ...
    // it has to be synchronous because it populates properties later used by the Marker object
	  let fourSquare = 'https://api.foursquare.com/v2/venues/search?ll='+ this.position.lat + ',' + this.position.lng + '&client_id=' + FOUR_SQUARE_CLIENT_ID + '&client_secret=' + FOUR_SQUARE_CLIENT_SECRET + '&v=20160118';
    ...
    this.popupInfo = function(location) { google.maps.event.trigger(self.marker, 'click'); };
...
function populatePopupWindow(marker) {
        ...
        let content = '<div>' +
                          '<span class="popupHeading">' + marker.title + '</span>'+
                          '<div>' + marker.street + '</div><div>' + marker.city + '</div>'
                      '</div>';
